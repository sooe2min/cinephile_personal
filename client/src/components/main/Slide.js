	...
	let a = [
		{
			...
			actors: '한지민, 그리고 남주혁',
			story:
				'자신을 ‘조제’로 불러달라는 그녀. 처음 만난 그날부터 조제는 영석에게 잊을 수 없는 이름으로 남는다.'
		},
		{
			image:
				'https://file.newswire.co.kr/data/datafile2/thumb_640/2020/11/3554238800_20201125150205_5963802919.jpg',
			title: '이웃사촌',
			actors: '정우, 오달수, 그리고 김희원',
			story:
				'적인가, 이웃인가? 수상한 가족들의 모든 소리와 행동을 감시하면서 새로운 비밀들을 하나씩 발견하게 되는데…'
		},
		{
			image:
				'https://t1.daumcdn.net/movie/99a4d42206a028764e86c5bff8cf0021db985223',
			title: '도굴',
			actors: '이제훈, 조우진',
			story:
				'고물인 줄 알았는데 보물이었다?! 땅 파서 장사하는 도굴꾼들의 팔수록 판이 커지는 급이 다른 삽질이 시작된다!'
		},
		{
			image:
			...
			story:
				'원인 불명의 재앙으로 종말을 맞이한 지구, 북극에 남겨진 과학자 오거스틴과 탐사를 마치고 지구로 귀환하던 우주 비행사 설리가 짧은 교신에 성공하는데…'
		},
		{
			image:
				'https://grizzlygrowler.org/wp-content/uploads/2020/12/Hulu-Run.jpg',
			title: 'Run',
			actors: 'Sarah Paulson',
			story:
				'장애 때문에 휠체어 생활을 하는 ‘클로이’. 어느 날 식탁에 놓인 장바구니에서 하나의 물건을 발견하게 되고 믿었던 모든 일상이 흔들리기 시작하는데…'
		}
	]
			...
			<div className="slide">
				...
				{a.map((el, idx) => (
					<div
						...
						style={{ display: `${array[idx].display}` }}
						key={idx}>
