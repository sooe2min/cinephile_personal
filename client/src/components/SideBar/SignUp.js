...
const SignUp = ({
	...
	let aouth = document.cookie
		.split('; ')
		.filter(string => string.slice(0, 8) === 'oauth_id')[0]
			...
			<form onSubmit={e => signUpHandler(e)}>
				...
				<input
					...
					onChange={e => {
						if (e.target.value.length > 6) {
							setCertificate(
								Object.assign({}, certificate, { username: '74b816' })
							)
						}
				...
				<FaChevronCircleDown
					className="checker"
					style={{ color: certificate.username }}
				/>
				<div className="selectWrapper">
					<select
						className="sidiBarSelect"
						defaultValue={'DEFAULT'}
						onChange={e => signUpSexSelect(e.target.value)}>
					...
					<select
						className="sidiBarSelect"
						defaultValue={'DEFAULT'}
						onChange={e => signUpAgeSelect(e.target.value)}>
