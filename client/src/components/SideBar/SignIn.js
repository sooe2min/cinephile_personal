	...
	return (
		<div className="modal" style={modalSwitch(loginSwitch)}>
			...
			<form onSubmit={e => singInHandler(e)}>
				<input
					...
					onChange={e => {
						if (e.target.value.length > 12) {
							setCertificate(
								Object.assign({}, certificate, { email: '74b816' })
							)
						}
				...
				<input
					...
					onChange={e => {
						if (e.target.value.length > 6) {
							setCertificate(
								Object.assign({}, certificate, { password: '74b816' })
							)
						}
				...
				<FaChevronCircleDown
					className="checker"
					style={{ color: certificate.password }}
				/>
			...
			<button className="sideBarButton" id="facebook">
				<a
					className="sotialLoginButton"
					href={`https://www.facebook.com/v9.0/dialog/oauth?client_id=${process.env.REACT_APP_APP_ID}&redirect_uri=${process.env.REACT_APP_REDIRECT_URI_FA}&state=test`}></a>
			</button>
			<button className="sideBarButton" id="kakao">
				<a
					className="sotialLoginButton"
					href={`https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${process.env.REACT_APP_REST_API_KEY}&redirect_uri=${process.env.REACT_APP_REDIRECT_URI_KA}`}></a>
			</button>
		</div>
	)
}
