...
const SideBar = ({
	...
	const cookie = document.cookie
		...
		.some(
			cookie =>
				...
				cookie.includes('cookie') ||
				cookie.includes('oauth_id')
		)
	...
	useEffect(() => {
		if (cookie) {
			...
			let sotialKey = document.cookie
				.split('; ')
				.filter(string => string.slice(0, 4) === 'user')[0]
				...
				<>
					<div className="loginAppear">
						...
						<span
							className="nickName"
							onClick={() =>
								!signUpSwitch && loginSwitch === false && loginModal()
							}>
							로그인을 해주세요.
						</span>
					</div>
					...
					<ul className="navUl">
						...
						<li
							className="sideBarList"
							onClick={() =>
								!loginSwitch && signUpSwitch === false && signUpModal()
							}>
							회원가입
						</li>
						<li className="sideBarList" to="/board">
							Board
						</li>
					</ul>
				</>
