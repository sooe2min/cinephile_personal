	...
	const modifySwitchHandler = e => {
		...
		if (e.target.className === 'nicknameModifyButton') {
			if (modifySwitch.nicknameModify && modifyNickname !== '') {
				...
				setModifySwitch(
					Object.assign({}, modifySwitch, {
						nicknameModify: !modifySwitch.nicknameModify
					})
				)
			} else {
				setModifySwitch(
		...
		if (e.target.className === 'passwordModifyButton') {
			if (modifySwitch.passwordModify && modifyPassword !== '') {
				...
				setModifySwitch(
					Object.assign({}, modifySwitch, {
						passwordModify: !modifySwitch.passwordModify
					})
				)
			} else {
				setModifySwitch(
		...
		if (e.target.className === 'emailModifyButton') {
			if (modifySwitch.emailModify && modifyEmail !== '') {
				...
				setModifySwitch(
					Object.assign({}, modifySwitch, {
						emailModify: !modifySwitch.emailModify
					})
				)
			} else {
				setModifySwitch(
		...
		if (e.target.className === 'submitModifyButton') {
			if (loginUserInfo) {
				modifyNickname === '' &&
					nickNameChangeInput(loginUserInfo.nickname)
	...
	return (
		loginUserInfo && (
			<div className="modifyUserInfo">
				...
				<p className="loginUserIntro">
					개인 정보를 수정하시려면 아래 수정 버튼을 클릭해 주세요.
				</p>
				<form action="" className="modifyTable">
					<label htmlFor="nickname" className="nickname">
						...
						{modifySwitch.nicknameModify ? (
							<input
								...
								onChange={e => nickNameChangeInput(e.target.value)}
							/>
						) : (
							<span className="modifyNicknameText">
								{modifyNickname ? modifyNickname : loginUserInfo.nickname}
							</span>
						)}
					</label>
					<button
						className="nicknameModifyButton"
						onClick={modifySwitchHandler}>
					...
					<label htmlFor="password" className="password">
						...
						{modifySwitch.passwordModify ? (
							<input
								...
								onChange={e => passwordChangeInput(e.target.value)}
							/>
						) : (
					...
					<label htmlFor="email" className="email">
						...
						{modifySwitch.emailModify ? (
							...
							<span className="modifyEmailText">
								{modifyEmail ? modifyEmail : loginUserInfo.email}
							</span>
						)}
					</label>
					<button
					...
					<label htmlFor="gender" className="gender">
						<select
							defaultValue={loginUserInfo.gender}
							onChange={e => genderChangeSelect(e.target.value)}>
					...
					<label htmlFor="age" className="ageModify">
						<select
							defaultValue={loginUserInfo.age}
							onChange={e => ageChangeSelect(e.target.value)}>
